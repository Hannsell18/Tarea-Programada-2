    ...
    public class Operaciones
    {
        ...
        /**
         * Permite modificar la ecuación para ser representada de forma correcta en el archivo de texto.
         * @param String ecuacion   Contiene la ecuacion inicial.
         * @param String respuesta  Contiene la respuesta de la ecuacion.
         * @return Contiene el string modificao.
         */
        public static String modificarCadena(String ecuacion, String respuesta){
            String dato = "";
            char temporal;
            for (int contador = 0; contador < ecuacion.length(); contador++){
                temporal = ecuacion.charAt(contador);
                if (temporal == "(".charAt(0) && contador > 0 || temporal == ")".charAt(0) && contador < ecuacion.length() - 1){
                    dato += " | ";
                }
                else if (temporal != "(".charAt(0) && temporal != ")".charAt(0)){
                    dato += Character.toString(temporal);
                }
            }
            dato += " | ";
            for (int contador = 0; contador < respuesta.length(); contador++){
                temporal = respuesta.charAt(contador);
        ...
        /**
         * Permite modificar la ecuación para ser utiliada, esta ecuación es obetenida de un .txt.
         * @param String ecuacion   Contiene la ecuacion inicial.
         * @return Contiene el string modificao.
         */
        public static String modificarCadena(String ecuacion){
            String dato = "";
            char temporal;
            for (int contador = 0; contador < ecuacion.length(); contador++){
                temporal = ecuacion.charAt(contador);
                if (contador == 0 || (temporal == "|".charAt(0) && "|".charAt(0) == ecuacion.charAt(contador - 2))){
                    if (temporal != "|".charAt(0)){
                        dato += "(" + ecuacion.charAt(contador);
                    }
                    else{
                        dato += "(";
                    }
                }
                else if (contador == ecuacion.length() - 1 ||
                        (temporal == "|".charAt(0) && "|".charAt(0) == ecuacion.charAt(contador + 2))){
                    if (temporal != "|".charAt(0)){
                        dato += ecuacion.charAt(contador) + ")";
                    }
                    else{
